...
import { signatureCocktails, imageById, galleryImages } from "@/data/mock";
    ...
    <section className="py-16">
      <div className="site-top reveal-grid mb-8 grid items-end gap-4 sm:grid-cols-2" data-anim="fade-up">
        ...
        <div className="flex flex-wrap items-center justify-end gap-3">
          <Input 
            placeholder="Išči koktajl" 
            value={query} 
            onChange={handleQueryChange} 
            className="w-48 bg-black/30 dark:bg-black/30" 
          />
          ...
          <Card key={c.id} className="reveal-card overflow-hidden border-white/10 bg-black/40 dark:bg-black/40">
            ...
            <CardContent className="space-y-2 p-5">
              <div className="flex items-center justify-between">
                ...
                <span className="rounded-md border border-primary/40 px-2 py-0.5 text-sm text-primary">
                  € {c.price}
                </span>
              </div>
              ...
              <div className="flex flex-wrap gap-2 pt-2">
                {c.tags.map((t) => (
                  <span 
                    key={t} 
                    className="rounded-full border border-white/10 px-2 py-0.5 text-[12px] text-muted-foreground"
                  >
                    {t}
                  </span>
                ))}
